 ...
 * @LastEditTime: 2022-03-08 10:31:55
...
function App() {
    ...
    const initP5 = useCallback(() => {
        ...
        new P5((sketch: P5) => {
            setSK(sketch)
            sketch.preload = () => {
                toolsManager.preload(sketch)
            }
            sketch.setup = () => {
                toolsManager.setup(sketch)
            }
            sketch.touchStarted = (event: any) => {
                if (event.target.nodeName === 'CANVAS' && !viewer?.isMouseNavEnabled) {
                    toolsManager.touchStarted(sketch)
                }
            }
            sketch.touchMoved = () => {
                toolsManager.touchMoved(sketch)
            }
            sketch.touchEnded = () => {
                toolsManager.touchEnded(sketch)
            }
        }, wrapDiv)
    }, [viewer, toolsManager])
